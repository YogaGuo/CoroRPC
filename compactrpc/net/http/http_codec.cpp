 ...
 * @LastEditors: Yogaguo
 * @LastEditTime: 2023-02-19 15:12:53
 */
/*
 * @Description:
 * @Version: 2.0
...
namespace compactrpc
{
    ...
    void HttpCodec::encode(TcpBuffer *buf, AbstractData *data)
    {
        ...
        buf->writeToBuffer(http_res.c_str(), http_res.size());
    ...
    {
        DebugLog << "test http_decode start";
        std::string strs;
        if (!buf || !data)
        {
        ...
        while (1)
        {
            if (!is_parse_request_line)
            {
                ...
                if (i == tmp.length() - 2)
                {
                    DebugLog << "need too read more data";
                    break;
                }
